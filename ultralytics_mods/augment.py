...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # check if img is a 5-channel image (RGB + custom)
            if img.shape[2] > 3:
                # check that we actually deal with 5-channel image data and not GRBA
                assert FIVE_CH_SUPP
                img_rgb = img[..., :3]
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if img.shape[-1] > 3:
            # check that we actually deal with 5-channel image data and not GRBA
            assert FIVE_CH_SUPP
            # Handle the first four channels
            img_4channels = img[:, :, :4]
            img_4channels = cv2.copyMakeBorder(
                img_4channels, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114, 114)
            )

            # Handle the fifth channel separately
            img_5th_channel = img[:, :, 4]
            img_5th_channel = cv2.copyMakeBorder(
                img_5th_channel, top, bottom, left, right, cv2.BORDER_CONSTANT, value=114
            )
            # Combine the 4-channel and 1-channel images back together
            img = np.dstack((img_4channels, img_5th_channel))
