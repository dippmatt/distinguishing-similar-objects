...
# Mask mode "patches" cuts out small patches of the original image
# while "masks" uses no texture information, only the masks generated by the SAM model
MASK_MODE = "patches"
#MASK_MODE = "masks"
...
def loop_iter(image_path, custom_image_dir):
    ...
    for i in range(masks.shape[0]):
        image[masks[i]] = gray[masks[i]]
    image = Image.fromarray(image)
    image.save(str(custom_image_dir / f"{image_path.stem}.jpg"))
    return
...
def process_images(original_image_dir: Path, custom_image_dir: Path):
    i = 0
    ...
    for image_path in tqdm.tqdm(sorted_images):
        loop_iter(image_path, custom_image_dir)
        i += 1
        continue
...
def _main():
    ...
    process_images(training_images_in, training_images_out)
    process_images(test_images_in, test_images_out)
